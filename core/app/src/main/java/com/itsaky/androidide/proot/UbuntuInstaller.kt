...
class UbuntuInstaller(private val context: Context) {
    ...
    private fun finalizeRootfs(onProgress: (InstallPhase) -> Unit) {
        ...
        File(rootfs, "etc/apt/apt.conf.d/99androidide").writeText(
            """
        ...
        File(rootfs, "etc/dpkg/dpkg.cfg.d/99androidide").writeText(
            "force-unsafe-io\nno-debsig\n"
        )
        ...
        File(rootfs, ProotConfig.INSTALL_MARKER).writeText(ProotConfig.ROOTFS_VERSION)
        ProotConfig.tarballFile(context).delete()
    }
}
